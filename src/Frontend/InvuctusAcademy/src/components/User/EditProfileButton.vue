    ...
    <q-card style="min-width: 350px">
      ...
      <q-form class="q-gutter-md" @submit="onSubmit" @reset="onReset">
        <q-card-section class="q-pt-none">
          ...
          <q-input dense mask="#(###) ### - ####" v-model="editData.phoneNumber" label="Телефонный номер" lazy-rules
            :rules="[
              (val) =>
                (val && val.length === 17) || 'Номер должен содержать 11 цифр',
            ]" />

          <q-input dense v-model="editData.instagramLink" label="Ссылка на Instagram" />

          <q-select v-model="editData.citizenship" :options="citizenships" label="Гражданство" />

          <q-input dense v-model="editData.city" label="Город" />
...
export default defineComponent({
  ...
  methods: {
    async getUserData() {
      ...
      this.editData.city = response.data.value.city
    },
    async onSubmit() {
      try {
        const autorize = await fetchLoginedUserData();
