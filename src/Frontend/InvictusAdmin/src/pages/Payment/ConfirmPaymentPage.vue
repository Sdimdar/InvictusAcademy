<template>
  ...
  const columns = [
      ...
      {name: 'rejectReason', align: 'center', label: 'Комментарий', field: 'rejectReason', sortable: false},
        ...
        onRequest      
          ...
          async reject(id,rejectMessage,index){
            ...
            try{
              ...
              if(response.data.isSuccess){
              notify.showSucsessNotify(`Оплата  для заявки ${id} отменена`)
