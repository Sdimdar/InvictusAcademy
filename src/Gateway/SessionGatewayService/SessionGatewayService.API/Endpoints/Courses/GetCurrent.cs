    ...
    .WithActionResult<DefaultResponceObject<CoursesVm>>
{
    ...
    public override async Task<ActionResult<DefaultResponceObject<CoursesVm>>> HandleAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            string email = HttpContext.Session.GetData("user")!.Email;
            var result = await _mediator.Send(new GetCoursesQuerry() { Email = email, Type = CourseTypes.Current }, cancellationToken);
